...
import com.globalrelay.mdscache.performance.feeder.RandomUserCapabilitiesFeeder.DATA
import com.globalrelay.mdscache.performance.helper.SimulationHelper.httpClient
import com.globalrelay.mdscache.performance.loader.GetUserFileCapabilitiesLoader.wireMockSession
import com.globalrelay.mdscache.performance.response.ResponseUtils.forGetUserFileCapabilities
import com.globalrelay.mdscache.performance.wiremock.WireMock.INSTANCE
import com.globalrelay.mdscache.wiremock.stubs.admin.GetUserFileCapabilitiesStub
import com.globalrelay.nucleus.wiremock.WireMockSession
import org.apache.http.client.methods.{CloseableHttpResponse, HttpGet}

import java.net.URLEncoder

class GetUserFileCapabilitiesLoader extends AbstractDataLoader {
  override def getWireMockSession: WireMockSession = {
    wireMockSession
  }

  override def loadJson(): Unit = {
    val json = ujson.read(os.read(os.resource / BASE_FEEDER_PATH / GET_USER_FILE_CAPABILITIES_FILE_NAME))
  ...
  private def send(id: String): Unit = {
    if (wireMockEnabled) {
      stubUserFileCapabilities(id)
    }

    val encodedId: String = URLEncoder.encode(id, "UTF-8")

    val get: HttpGet = new HttpGet(BASE_URL + "/users/" + encodedId + "/file-capabilities")
  ...
  private def stubUserFileCapabilities(id: String): Unit = {
    getWireMockSession.register(new GetUserFileCapabilitiesStub(id)
      .withResponseBody(forGetUserFileCapabilities()))
  }
}

object GetUserFileCapabilitiesLoader {
